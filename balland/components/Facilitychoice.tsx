...
const FacilityChoice = (props: FacilityProps) => {
  ...
  <div className="container mx-auto p-9 bg-white max-w-sm rounded-2xl overflow-hidden shadow-xl hover:shadow-2xl transition duration-300">
    ...
    <div className="flex justify-between items-center">
      <div>
        <h1 className="mt-[20px] text-[10px] xxs:text-[11px] xs:text-[14px] s:text-[20px] sm:text-[24px] font-semibold">{props.Facility}</h1>
      </div>
      <div>
        <button 
            onClick =  {() => Router.push("/reservation/bigground")}
            className="text-white mt-[20px] text-[10px] xxs:text-[10px] xs:text-[14px] s:text-[18px] font-semibold bg-blue-400 py-2 px-4 rounded-lg shadow-md hover:shadow-lg transition duration-500 transform-gpu hover:scale-110 ">예약 하기</button>
      </div>
    </div>
  </div>
</div>
